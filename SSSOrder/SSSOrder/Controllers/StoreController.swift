...
//                            let owners = item["Owners"] as? [[String:Any]]
                        ...
                        let salon = SalonStoreModel(salonId: id, name: name, address: address, description: description != nil ? description!:"", distance: distance, image: image, latitude: latP, longitude: longP)
                        list.append(salon)
                    }
                    callback(list, nil)
                } catch {
