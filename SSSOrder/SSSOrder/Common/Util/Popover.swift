...
open class Popover: UIView {
  ...
  open func show(_ contentView: UIView, fromView: UIView, inView: UIView) {
    ...
    open func show(_ contentView: UIView, fromView: UIView, inScrollView: UIScrollView) {
        guard let rootView = UIApplication.shared.windows.last ?? UIApplication.shared.keyWindow else {
            return
        }
        self.show(contentView, fromView: fromView, inScrollView: inScrollView, inView: rootView)
    }

    open func show(_ contentView: UIView, fromView: UIView, inScrollView: UIScrollView, inView: UIView) {
        let point: CGPoint

//        var xPoint = fromView.frame.origin.x + (fromView.frame.size.width / 2)
//        let realPoint = inScrollView.convert(CGPoint(x: xPoint, y: fromView.frame.origin.y), to: nil)
//        if realPoint.x <= 25 {
//            xPoint = fromView.frame.origin.x + fromView.frame.size.width
//        }
//        
//        if realPoint.x > ScreenConstant.SCREEN_WIDTH {
//            xPoint = fromView.frame.origin.x
//        }

        let xPoint = checkPositionForView(fromView: fromView, inScrollView: inScrollView)

        switch self.popoverType {
        case .up:
            point = inView.convert(CGPoint(x: xPoint, y: fromView.frame.origin.y), from: fromView.superview)
        case .down:
            point = inView.convert(CGPoint(x: xPoint, y: fromView.frame.origin.y + fromView.frame.size.height), from: fromView.superview)
        }
    ...
    fileprivate func checkPositionForView(fromView: UIView, inScrollView: UIScrollView) -> CGFloat {
